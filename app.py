...
st.markdown("**Oleh: Henni Marsella (06111282429044)**")
...
if uploaded_file is not None:
    ...
    # ==========================================================
    # RANKING SOAL (DENGAN FALLBACK JIKA TOTAL SKOR KONSTAN)
    # ==========================================================
    st.subheader("📊 Ranking Soal Berpengaruh terhadap Total Skor")
    
    if df['Total_Skor'].std() > 0:
        corr_total = indikator.corrwith(df['Total_Skor']).sort_values(ascending=False)
        ranking_df = pd.DataFrame({
            'Nomor Soal': [int(col.split('_')[1]) for col in corr_total.index],
            'Koefisien Korelasi': corr_total.values.round(3)
        })
        st.dataframe(ranking_df.head(10), use_container_width=True, hide_index=True)
        st.caption("Nilai mendekati 1 = hubungan positif kuat dengan total skor")
    else:
        st.warning("⚠️ Total skor semua siswa identik, korelasi tidak dapat dihitung.")
        st.info("📊 Alternatif: Ranking berdasarkan rata-rata skor per soal")
        ranking_soal = mean_scores.sort_values(ascending=False)
        ranking_df = pd.DataFrame({
            'Nomor Soal': [int(col.split('_')[1]) for col in ranking_soal.index],
            'Rata-rata Skor': ranking_soal.values.round(3),
            'Kategori': ['Baik' if x >= 3.0 else ('Cukup' if x >= 2.0 else 'Perlu Perbaikan') for x in ranking_soal.values]
        })
        st.dataframe(ranking_df.head(10), use_container_width=True, hide_index=True)
    ...
    **Kategori:**
    ...
    if df['Total_Skor'].std() > 0:
        corr_total = indikator.corrwith(df['Total_Skor'])
        top_corr = corr_total.idxmax().split('_')[1]
        r_val = corr_total.max()
    else:
        top_corr = mean_scores.idxmax().split('_')[1]
        r_val = mean_scores.max()
    ...
    3. **Korelasi Kuat**: Soal dengan kinerja terbaik adalah Soal_{top_corr} (rata-rata: {r_val:.3f}).
